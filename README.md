...
`사용자가 촬영하거나 업로드한 계약서 이미지를 분석한다.`

- **[예외 상황]**
    - [x] 업로드 된 이미지의 크기가 20MB를 초과하는 경우
    - [x] 이미지 파일이 아닌 다른 형식의 파일이 업로드된 경우
    - [x] AI 모델의 응답 지연이나 API 호출 할당량이 초과된 경우
    - [x] AI 의 응답이 json 양식에 일치하지 않는 경우

`비회원 or 회원에 따라 분석 요청을 제한한다.`

- **[예외 상황]**
    - [x] 비회원이 일일 3회 이상 분석을 요청한 경우

**[분석 저장]**

`로그인된 사용자는 분석된 결과를 저장하거나 조회할 수 있다.`
